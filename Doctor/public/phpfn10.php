...
function CurrentProjectID() {
	...
	return "{AC1E48A3-8A06-46C6-A848-8ACFB1B7C356}";
}
	...
	if ($fn <> "") {
		if ($fld->IsBlobImage) {
			...
			if ($fld->HrefValue == "")
				...
				return "<a href=\"" . $fld->HrefValue . "\"" . $fld->LinkAttributes() . "><img src=\"" . $fn . "\" alt=\"\"" . $fld->ViewAttributes() . "></a>";
		} else {
			if ($fld->FldDataType == EW_DATATYPE_BLOB) {
				return ($fld->Upload->FileName <> "") ? $fld->Upload->FileName : $fld->FldCaption();
			} else {
				$fn = ew_ImageNameFromUrl($fn);
				return "<div><a href=\"" . $fn . "\"" . $fld->LinkAttributes() . ">" . basename($fn) . "</a></div>";
			}
		}
	} else {
		return "";
